...
export function SectionPreviewContent({
  variant,
  setVariant,
  files,
  activeFile,
  ...
  useEffect(() => {
    if (width !== undefined) setInternalWidth(width);
  }, [width]);
  ...
  const handleOpenWindow = () => {
    if (sectionId && variantIndex !== undefined) {
      openWindow({
        categoryId: 'sections',
        exampleId: `${sectionId}-${variantIndex}`,
        width: 1200,
        height: 900,
      });
    }
    ...
    <div className="w-full max-w-5xl h-full overflow-hidden">
      ...
      >
        {variant === 'preview' && DemoComponent ? (
          <div className="border-x border-background-700 w-full">
            <DemoComponent />
          </div>
        ) : (
