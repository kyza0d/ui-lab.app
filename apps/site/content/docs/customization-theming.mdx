...
<MermaidDiagram chart={`graph TB
    ...
    subgraph Families["Color Families"]
        BG["Background 500-950"]
        FG["Foreground 50-400"]
        ACC["Accent 50-600"]
        SEM["Semantic: Success, Danger, Warning, Info"]
    end

    subgraph Tokens["CSS Custom Properties"]
        BGVAR["--background-N"]
        FGVAR["--foreground-N"]
        ACCVAR["--accent-N"]
        SEMVAR["--success-N, --danger-N, --warning-N"]
    end

    subgraph Usage["Authoring Layer"]
        TW["Tailwind: bg-accent-600, text-foreground-300"]
        CSSV["CSS: var(--accent-600)"]
        DM["Dark Mode: shades auto-invert"]
    end
    ...
    ACC --> ACCVAR
    SEM --> SEMVAR
    BGVAR & FGVAR & ACCVAR & SEMVAR --> TW & CSSV
    TW & CSSV --> DM
`} />
...
The accent family is designed for dynamic customization. Override it in your CSS:
